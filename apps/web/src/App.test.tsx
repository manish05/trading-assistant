...
describe('Dashboard shell', () => {
  ...
  it('renders gateway action buttons for account/feed listing', () => {
    ...
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Basis Transition Summary')).toHaveTextContent(
      'Active neighbor basis transition: none',
    )
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Delta Trend Summary')).toHaveTextContent(
      'Active neighbor delta trend: none',
    )
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Delta Acceleration Summary')).toHaveTextContent(
      'Active neighbor delta acceleration: none',
    )
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Tone Transition Summary')).toHaveTextContent(
      'Active neighbor tone transition: none',
    )
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Consensus Summary')).toHaveTextContent(
      'Active neighbor consensus: none',
    )
    ...
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Consensus Volatility Summary')).toHaveTextContent(
      'Active neighbor consensus volatility: none',
    )
    ...
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Polarity Transition Summary')).toHaveTextContent(
      'Active neighbor polarity transition: none',
    )
    ...
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Vote Summary')).toHaveTextContent(
      'Active neighbor magnitude side vote: none',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Transition Summary'),
    ).toHaveTextContent('Active neighbor magnitude side transition: none')
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Drift Summary'),
    ).toHaveTextContent('Active neighbor magnitude side drift: none')
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Regime Summary'),
    ).toHaveTextContent('Active neighbor magnitude side regime: none')
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pressure Summary'),
    ).toHaveTextContent('Active neighbor magnitude side pressure: none')
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Certainty Summary'),
    ).toHaveTextContent('Active neighbor magnitude side certainty: none')
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Coherence Summary'),
    ).toHaveTextContent('Active neighbor magnitude side coherence: none')
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Vector Summary'),
    ).toHaveTextContent('Active neighbor magnitude side vector: none')
    expect(
    ...
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Conflict Score Summary'),
    ).toHaveTextContent('Active neighbor magnitude side conflict score: none')
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pulse Summary'),
    ).toHaveTextContent('Active neighbor magnitude side pulse: none')
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pulse Transition Summary'),
    ).toHaveTextContent('Active neighbor magnitude side pulse transition: none')
    expect(
    ...
    expect(screen.getByLabelText('Overlay Marker Event Candle Comparative Drift Summary')).toHaveTextContent(
      'Marker event candle comparative drift: none',
    )
    expect(screen.getByLabelText('Overlay Marker Event Timeline Cadence Summary')).toHaveTextContent(
      'Marker event timeline cadence: none',
    )
    ...
    expect(screen.getByLabelText('Overlay Marker Visual Context Drift Summary')).toHaveTextContent(
      'Marker visual context drift: none',
    )
    ...
    expect(screen.getByLabelText('Overlay Marker Visual Neighbor Cadence Drift Summary')).toHaveTextContent(
      'Marker visual neighbor cadence drift: none',
    )
    ...
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Pressure Summary')).toHaveTextContent(
      'Timeline interaction pressure: none',
    )
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Readiness Summary')).toHaveTextContent(
      'Timeline interaction readiness: none',
    )
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Coherence Summary')).toHaveTextContent(
      'Timeline interaction coherence: none',
    )
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Vector Summary')).toHaveTextContent(
      'Timeline interaction vector: none',
    )
    ...
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Parity Summary')).toHaveTextContent(
      'Timeline interaction parity: none',
    )
  ...
  it('updates overlay legend and live summary by selected mode and events', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByLabelText('Overlay Marker Active Neighbor Basis Transition Summary')).toHaveTextContent(
        'Active neighbor basis transition: active:diverge · prev:agree · next:n/a · transition:prev->active:enter-diverge|active->next:n/a · basis:latest · mode:all',
      )
      expect(screen.getByLabelText('Overlay Marker Active Neighbor Delta Trend Summary')).toHaveTextContent(
        'Active neighbor delta trend: active:risk:live_trading_disabled:raised · latest:prev->active:improving(+1.00)|active->next:n/a · average:prev->active:improving(+1.00)|active->next:n/a · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Delta Acceleration Summary'),
      ).toHaveTextContent(
        'Active neighbor delta acceleration: active:risk:live_trading_disabled:raised · prev->active:latest:+1.00|average:+1.00|accel:+0.00 · active->next:latest:n/a|average:n/a|accel:n/a · basis:latest · mode:all',
      )
      expect(screen.getByLabelText('Overlay Marker Active Neighbor Tone Transition Summary')).toHaveTextContent(
        'Active neighbor tone transition: active:risk:live_trading_disabled:raised · latest:prev->active:down->flat|active->next:n/a · average:prev->active:down->up|active->next:n/a · basis:latest · mode:all',
      )
      expect(screen.getByLabelText('Overlay Marker Active Neighbor Consensus Summary')).toHaveTextContent(
        'Active neighbor consensus: active:risk:live_trading_disabled:raised · prev:agree · active:diverge · next:n/a · counts:agree:1|diverge:1 · majority:tie · basis:latest · mode:all',
      )
      expect(screen.getByLabelText('Overlay Marker Active Neighbor Consensus Balance Summary')).toHaveTextContent(
      ...
      expect(screen.getByLabelText('Overlay Marker Active Neighbor Consensus Volatility Summary')).toHaveTextContent(
        'Active neighbor consensus volatility: active:risk:live_trading_disabled:raised · transitions:prev->active:toward-diverge(-2)|active->next:toward-agree(+1) · swing:3 · basis:latest · mode:all',
      )
      expect(screen.getByLabelText('Overlay Marker Active Neighbor Delta Relation Summary')).toHaveTextContent(
      ...
      expect(screen.getByLabelText('Overlay Marker Active Neighbor Polarity Transition Summary')).toHaveTextContent(
        'Active neighbor polarity transition: active:risk:live_trading_disabled:raised · latest:prev->active:neg->zero|active->next:n/a · average:prev->active:neg->pos|active->next:n/a · basis:latest · mode:all',
      )
      expect(screen.getByLabelText('Overlay Marker Active Neighbor Polarity Shift Summary')).toHaveTextContent(
      ...
      expect(screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Vote Summary')).toHaveTextContent(
        'Active neighbor magnitude side vote: active:risk:live_trading_disabled:raised · latest:side:none|available:0/2 · average:side:prev|available:1/2 · combined:prev|vote:+1 · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Transition Summary'),
      ).toHaveTextContent(
        'Active neighbor magnitude side transition: active:risk:live_trading_disabled:raised · latest:none(0/2) · average:prev(1/2) · transition:none->prev|toward-prev(+1) · cross-basis:partial · net:prev|strength:1/1 · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Drift Summary'),
      ).toHaveTextContent(
        'Active neighbor magnitude side drift: active:risk:live_trading_disabled:raised · latest:vote:+0|side:none|available:0/2 · average:vote:+1|side:prev|available:1/2 · drift:+1|toward-prev · dominance:average · alignment:partial · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Regime Summary'),
      ).toHaveTextContent(
        'Active neighbor magnitude side regime: active:risk:live_trading_disabled:raised · latest:side:none|available:0/2|confidence:0.00 · average:side:prev|available:1/2|confidence:0.50 · regime:emerging|agreement:split|dominant:average · confidenceGap:-0.50|coverage:0.25 · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pressure Summary'),
      ).toHaveTextContent(
        'Active neighbor magnitude side pressure: active:risk:live_trading_disabled:raised · latest:pressure:+0.00|side:none|confidence:0.00 · average:pressure:+0.50|side:prev|confidence:0.50 · net:+0.50|bias:prev|conviction:moderate · drift:+0.50|coherence:ramping · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Certainty Summary'),
      ).toHaveTextContent(
        'Active neighbor magnitude side certainty: active:risk:live_trading_disabled:raised · latest:certainty:0.00|uncertainty:1.00|state:none · average:certainty:0.50|uncertainty:0.50|state:prev · combined:certainty:0.25|uncertainty:0.75|class:uncertain|consensus:split · drift:-0.50|tightening · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Coherence Summary'),
      ).toHaveTextContent(
        'Active neighbor magnitude side coherence: active:risk:live_trading_disabled:raised · latest:weighted:+0|side:none|available:0/2 · average:weighted:+1|side:prev|available:1/2 · coherence:+1.00|label:prev-lean · dispersion:1|coupling:assistive · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Vector Summary'),
      ).toHaveTextContent(
        'Active neighbor magnitude side vector: active:risk:live_trading_disabled:raised · latest:none|vector:+0 · average:prev|vector:+1 · vector:+1|normalized:+0.50|bias:prev · resolved:1/2|stability:forming · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Entropy Summary'),
      ).toHaveTextContent(
      ...
      expect(
        ...
        'Active neighbor magnitude side conflict score: active:risk:live_trading_disabled:raised · latest:none|score:+0 · average:prev|score:+1 · conflict:0.50|band:moderate|resolution:one-sided · coverage:1/2 · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pulse Summary'),
      ).toHaveTextContent(
        'Active neighbor magnitude side pulse: active:risk:live_trading_disabled:raised · latest:none|score:+0 · average:prev|score:+1 · pulse:+1|variance:1|coverage:1/2 · phase:building|bias:prev|intensity:medium · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pulse Transition Summary'),
      ).toHaveTextContent(
        'Active neighbor magnitude side pulse transition: active:risk:live_trading_disabled:raised · latest:none|score:+0 · average:prev|score:+1 · transition:none->prev|step:+1 · pulse:+1|slope:toward-prev · basis:latest · mode:all',
      )
      expect(
        screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pulse Alignment Summary'),
      ).toHaveTextContent(
      ...
      expect(screen.getByLabelText('Overlay Marker Event Candle Comparative Drift Summary')).toHaveTextContent(
        'Marker event candle comparative drift: active:risk:live_trading_disabled:raised · basis:latest:+0.00|alt:+0.50|drift:-0.50|agreement:diverge · neighbors:prevSlope:+1.00|nextSlope:n/a|vector:+1.00|direction:warming|coverage:1/2 · navBias:backward|home:on|end:off · mode:all',
      )
      expect(screen.getByLabelText('Overlay Marker Event Timeline Cadence Summary')).toHaveTextContent(
        'Marker event timeline cadence: active:risk:live_trading_disabled:raised|slot:2/2 · prev:step:1|marker:1|weight:1.00 · next:step:n/a|marker:n/a|weight:n/a · cadence:one-sided|balance:n/a|direction:backward|phase:backward-sync|coverage:1/2 · basis:latest · mode:all',
      )
      expect(screen.getByLabelText('Overlay Marker Event Timeline Coupling Summary')).toHaveTextContent(
      ...
      expect(screen.getByLabelText('Overlay Marker Visual Context Drift Summary')).toHaveTextContent(
        'Marker visual context drift: activeTone:flat|score:+0 · context:0|agreement:0|opposition:0|neutral:0 · drift:n/a|pressure:none|phase:isolated · basis:latest · mode:all',
      )
      expect(screen.getByLabelText('Overlay Marker Visual Role Pulse Summary')).toHaveTextContent(
      ...
      expect(screen.getByLabelText('Overlay Marker Timeline Interaction Pressure Summary')).toHaveTextContent(
        'Timeline interaction pressure: active:risk:live_trading_disabled:raised|slot:2/2 · pressure:backward:2|forward:0|net:+2|bias:backward|intensity:strong · lanes:step:backward|kind:idle|bucket:idle · edges:home:on|end:off|coverage:2/10|phase:backward-lean · wrap:bounded|order:newest-first',
      )
      expect(screen.getByLabelText('Overlay Marker Timeline Interaction Readiness Summary')).toHaveTextContent(
        'Timeline interaction readiness: active:risk:live_trading_disabled:raised|slot:2/2 · matrix:step:1/0|skip:0/0|kind:0/0|bucket:0/0|edge:1/0 · readiness:backward:2|forward:0|total:2/10|tilt:backward|band:light · wrap:bounded|selection:sticky|order:newest-first',
      )
      expect(screen.getByLabelText('Overlay Marker Timeline Interaction Coherence Summary')).toHaveTextContent(
        'Timeline interaction coherence: active:risk:live_trading_disabled:raised|slot:2/2 · readiness:b2/f0|route:1/6|edge:1/2 · net:+2|bias:backward|coherence:edge-lean|phase:backward-sync · wrap:bounded|selection:sticky|order:newest-first',
      )
      expect(screen.getByLabelText('Overlay Marker Timeline Interaction Vector Summary')).toHaveTextContent(
        'Timeline interaction vector: active:risk:live_trading_disabled:raised|slot:2/2 · vectors:backward:2|forward:0|net:+2|lane:+1|edge:+1 · state:backward|stability:firm|coverage:2/10|band:narrow · wrap:bounded|selection:sticky|order:newest-first',
      )
      expect(screen.getByLabelText('Overlay Marker Timeline Interaction Entropy Summary')).toHaveTextContent(
      ...
      expect(screen.getByLabelText('Overlay Marker Timeline Interaction Parity Summary')).toHaveTextContent(
        'Timeline interaction parity: active:risk:live_trading_disabled:raised|slot:2/2 · parity:min:0|max:2|ratio:0.00|state:backward|phase:one-sided · coverage:2/10|class:light · edges:home:on|end:off · wrap:bounded|selection:sticky|order:newest-first',
      )
      expect(screen.getByLabelText('Overlay Marker Behavior')).toHaveTextContent(
        'Marker behavior: wrap:bounded · selection:sticky · nav:manual',
      )
    ...
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Pressure Summary')).toHaveTextContent(
      'Timeline interaction pressure: active:trade:closed:queued|slot:1/2 · pressure:backward:0|forward:2|net:-2|bias:forward|intensity:strong · lanes:step:forward|kind:idle|bucket:idle · edges:home:off|end:on|coverage:2/10|phase:forward-lean · wrap:bounded|order:newest-first',
    )
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Readiness Summary')).toHaveTextContent(
      'Timeline interaction readiness: active:trade:closed:queued|slot:1/2 · matrix:step:0/1|skip:0/0|kind:0/0|bucket:0/0|edge:0/1 · readiness:backward:0|forward:2|total:2/10|tilt:forward|band:light · wrap:bounded|selection:sticky|order:newest-first',
    )
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Coherence Summary')).toHaveTextContent(
      'Timeline interaction coherence: active:trade:closed:queued|slot:1/2 · readiness:b0/f2|route:1/6|edge:1/2 · net:-2|bias:forward|coherence:edge-lean|phase:forward-sync · wrap:bounded|selection:sticky|order:newest-first',
    )
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Vector Summary')).toHaveTextContent(
      'Timeline interaction vector: active:trade:closed:queued|slot:1/2 · vectors:backward:0|forward:2|net:-2|lane:-1|edge:-1 · state:forward|stability:firm|coverage:2/10|band:narrow · wrap:bounded|selection:sticky|order:newest-first',
    )
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Entropy Summary')).toHaveTextContent(
    ...
    expect(screen.getByLabelText('Overlay Marker Timeline Interaction Parity Summary')).toHaveTextContent(
      'Timeline interaction parity: active:trade:closed:queued|slot:1/2 · parity:min:0|max:2|ratio:0.00|state:forward|phase:one-sided · coverage:2/10|class:light · edges:home:off|end:on · wrap:bounded|selection:sticky|order:newest-first',
    )
    expect(screen.getByRole('button', { name: 'trade:closed:queued' })).toHaveAttribute(
      'aria-pressed',
      'true',
    ...
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Basis Transition Summary')).toHaveTextContent(
      'Active neighbor basis transition: active:agree · prev:n/a · next:diverge · transition:prev->active:n/a|active->next:enter-diverge · basis:latest · mode:all',
    )
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Delta Trend Summary')).toHaveTextContent(
      'Active neighbor delta trend: active:trade:closed:queued · latest:prev->active:n/a|active->next:improving(+1.00) · average:prev->active:n/a|active->next:improving(+1.00) · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Delta Acceleration Summary'),
    ).toHaveTextContent(
      'Active neighbor delta acceleration: active:trade:closed:queued · prev->active:latest:n/a|average:n/a|accel:n/a · active->next:latest:+1.00|average:+1.00|accel:+0.00 · basis:latest · mode:all',
    )
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Tone Transition Summary')).toHaveTextContent(
      'Active neighbor tone transition: active:trade:closed:queued · latest:prev->active:n/a|active->next:down->flat · average:prev->active:n/a|active->next:down->up · basis:latest · mode:all',
    )
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Consensus Summary')).toHaveTextContent(
      'Active neighbor consensus: active:trade:closed:queued · prev:n/a · active:agree · next:diverge · counts:agree:1|diverge:1 · majority:tie · basis:latest · mode:all',
    )
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Consensus Balance Summary')).toHaveTextContent(
    ...
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Consensus Volatility Summary')).toHaveTextContent(
      'Active neighbor consensus volatility: active:trade:closed:queued · transitions:prev->active:toward-agree(+1)|active->next:toward-diverge(-2) · swing:3 · basis:latest · mode:all',
    )
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Delta Relation Summary')).toHaveTextContent(
    ...
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Polarity Transition Summary')).toHaveTextContent(
      'Active neighbor polarity transition: active:trade:closed:queued · latest:prev->active:n/a|active->next:neg->zero · average:prev->active:n/a|active->next:neg->pos · basis:latest · mode:all',
    )
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Polarity Shift Summary')).toHaveTextContent(
    ...
    expect(screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Vote Summary')).toHaveTextContent(
      'Active neighbor magnitude side vote: active:trade:closed:queued · latest:side:next|available:1/2 · average:side:next|available:1/2 · combined:next|vote:-2 · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Transition Summary'),
    ).toHaveTextContent(
      'Active neighbor magnitude side transition: active:trade:closed:queued · latest:next(1/2) · average:next(1/2) · transition:next->next|flat · cross-basis:agree · net:next|strength:2/2 · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Drift Summary'),
    ).toHaveTextContent(
      'Active neighbor magnitude side drift: active:trade:closed:queued · latest:vote:-1|side:next|available:1/2 · average:vote:-1|side:next|available:1/2 · drift:+0|stable · dominance:balanced · alignment:aligned · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Regime Summary'),
    ).toHaveTextContent(
      'Active neighbor magnitude side regime: active:trade:closed:queued · latest:side:next|available:1/2|confidence:0.50 · average:side:next|available:1/2|confidence:0.50 · regime:locked|agreement:same|dominant:balanced · confidenceGap:+0.00|coverage:0.50 · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pressure Summary'),
    ).toHaveTextContent(
      'Active neighbor magnitude side pressure: active:trade:closed:queued · latest:pressure:-0.50|side:next|confidence:0.50 · average:pressure:-0.50|side:next|confidence:0.50 · net:-1.00|bias:next|conviction:strong · drift:+0.00|coherence:synchronized · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Certainty Summary'),
    ).toHaveTextContent(
      'Active neighbor magnitude side certainty: active:trade:closed:queued · latest:certainty:0.50|uncertainty:0.50|state:next · average:certainty:0.50|uncertainty:0.50|state:next · combined:certainty:0.50|uncertainty:0.50|class:developing|consensus:same · drift:+0.00|flat · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Coherence Summary'),
    ).toHaveTextContent(
      'Active neighbor magnitude side coherence: active:trade:closed:queued · latest:weighted:-1|side:next|available:1/2 · average:weighted:-1|side:next|available:1/2 · coherence:-1.00|label:next-lean · dispersion:0|coupling:coupled · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Vector Summary'),
    ).toHaveTextContent(
      'Active neighbor magnitude side vector: active:trade:closed:queued · latest:next|vector:-1 · average:next|vector:-1 · vector:-2|normalized:-1.00|bias:next · resolved:2/2|stability:locked · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Entropy Summary'),
    ).toHaveTextContent(
    ...
    expect(
      ...
      'Active neighbor magnitude side conflict score: active:trade:closed:queued · latest:next|score:-1 · average:next|score:-1 · conflict:0.00|band:aligned|resolution:same-direction · coverage:2/2 · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pulse Summary'),
    ).toHaveTextContent(
      'Active neighbor magnitude side pulse: active:trade:closed:queued · latest:next|score:-1 · average:next|score:-1 · pulse:-2|variance:0|coverage:2/2 · phase:locked|bias:next|intensity:high · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pulse Transition Summary'),
    ).toHaveTextContent(
      'Active neighbor magnitude side pulse transition: active:trade:closed:queued · latest:next|score:-1 · average:next|score:-1 · transition:hold:next|step:+0 · pulse:-2|slope:flat · basis:latest · mode:all',
    )
    expect(
      screen.getByLabelText('Overlay Marker Active Neighbor Magnitude Side Pulse Alignment Summary'),
    ).toHaveTextContent(
    ...
    expect(screen.getByLabelText('Overlay Marker Event Candle Comparative Drift Summary')).toHaveTextContent(
      'Marker event candle comparative drift: active:trade:closed:queued · basis:latest:-1.00|alt:-0.50|drift:-0.50|agreement:agree · neighbors:prevSlope:n/a|nextSlope:+1.00|vector:+1.00|direction:warming|coverage:1/2 · navBias:forward|home:off|end:on · mode:all',
    )
    expect(screen.getByLabelText('Overlay Marker Event Timeline Cadence Summary')).toHaveTextContent(
      'Marker event timeline cadence: active:trade:closed:queued|slot:1/2 · prev:step:n/a|marker:n/a|weight:n/a · next:step:1|marker:1|weight:1.00 · cadence:one-sided|balance:n/a|direction:forward|phase:forward-sync|coverage:1/2 · basis:latest · mode:all',
    )
    expect(screen.getByLabelText('Overlay Marker Event Timeline Coupling Summary')).toHaveTextContent(
    ...
    expect(screen.getByLabelText('Overlay Marker Visual Context Drift Summary')).toHaveTextContent(
      'Marker visual context drift: activeTone:down|score:-1 · context:0|agreement:0|opposition:0|neutral:0 · drift:n/a|pressure:none|phase:isolated · basis:latest · mode:all',
    )
    expect(screen.getByLabelText('Overlay Marker Visual Role Pulse Summary')).toHaveTextContent(
